  ...
  return (
    <div className="w-full h-[100%] " >
      <nav className="hidden md:flex w-full border-b border-gray-300 bg-white px-4 md:px-8 py-4 items-center  flex justify-between">
        ...
        <div onClick={() => navigate(`/profile/${userData.userId}`)} className="flex items-center gap-2" >
          <div className="flex items-center gap-2 h-11 border-1 border-[#b08969ff] rounded-lg px-2 cursor-pointer" >
            ...
            <p className="text-sm font-medium">{userData.userName ? `topmate.io/${userData.userName}` : 'Profile URL'}</p>
            <FiExternalLink size={20} color="#b08969ff" className="cursor-pointer " />
          </div>
          <div className="flex items-center gap-2 h-11 border-1 border-[#b08969ff] rounded-lg px-2 cursor-pointer">
            <FiCopy size={20} color="#b08969ff" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/profile/${userData.userId}`); toast.success('Profile URL copied!'); }} />
          </div>
        </div>
